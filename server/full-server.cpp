...
public:
    ConnectionHolder(crow::websocket::connection& connection)
...
InteractiveTickResult tick_interactive_game(InteractiveGame const* game, InteractiveGameState* game_data, PlayerIdentifier called_player_id, std::vector<std::string>& called_player_errors)
{
    ...
    if (game_data->game_in_progress()) {
        ...
        if (game_tick_result.has_error()) {
            std::cout << "Got error: " << game_tick_result.error() << '\n';
            if (called_player_id == game_tick_result.get_next_player()) {
                called_player_errors.emplace_back(game_tick_result.extract_string());
            } else {
                game_data->add_error(game_tick_result.get_next_player(), game_tick_result.extract_string());
            }
        }

        if (!game_tick_result.is_done()) {
            if (game_tick_result.get_next_player() == called_player_id)
    ...
    std::vector<std::string> run(bool failed = false)
    {
        ...
        if (!game_data->can_run_exclusive()) {
            ...
            if (failed) {
                ...
                auto result = tick_interactive_game(game, game_data, *player_id, errors);
                ASSERT((result == InteractiveTickResult::FailedCleanUpState)
                    || (result == InteractiveTickResult::FailedClearStateOnly));

                game_data->move_errors(*player_id, errors);
        ...
        std::cout << "Ticking with input: '" << input_string << "'\n";
        ...
        // Anything interactive done with the game data / players should be done while the game is "running"
        {
            ...
            if (result == InteractiveTickResult::WaitingOnYou || result == InteractiveTickResult::Running) {
                std::vector<std::string> messages_out { output_buffer.size() };
                std::move(output_buffer.begin(), output_buffer.end(), messages_out.begin());
                output_buffer.clear();
                return messages_out;
            }
        }
...
void handle_ws_message(crow::websocket::connection& conn, std::string input, bool closed = false)
{
    ...
    if (!input.empty() && !closed) {
        ...
        // std::cout.flush();
        ws_state->observers.send_message(
            crow::json::wvalue {
                { "type", "user-message" },
                { "content", input },
            }
    ...
    for (auto& str : ws_state->errors) {
        push_message("error", std::move(str));
    }
    ws_state->errors.clear();
...
int main()
{
    ...
    CROW_ROUTE(app, "/api/game-info/<string>")
    ([](std::string game) {
    ...
    CROW_ROUTE(app, "/api/setup-game/<string>")
        .methods(crow::HTTPMethod::POST)([](crow::request const& req, std::string game) -> crow::response {
